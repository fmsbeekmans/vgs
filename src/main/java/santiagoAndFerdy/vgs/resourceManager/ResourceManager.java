...
public class ResourceManager extends UnicastRemoteObject implements IResourceManager {
    ...
    public ResourceManager(RmiServer rmiServer, int id, IRepository<IUser> userRepository, // TODO use appendable user repository.
            IRepository<IResourceManager> resourceManagerRepository, IRepository<IGridScheduler> gridSchedulerRepository, int nNodes)
            ...
            synchronized (monitoredAt) {
                ...
                needNewPrimary.forEach(req -> {
                    ...
                    if (newMonitor.isPresent()) {
                        try { //TODO backup selection
                            newMonitor.get().promote(new MonitoringRequest(id, req, selectBackUpGridSchedule(newMonitor.get().getId())));
    ...
    public void offerWork(WorkRequest req) throws RemoteException {
        if (!running)
            throw new RuntimeException("I am offline");
            ...
            if (needToOffload()) {
                ...
                if (monitorGridScheduler.isPresent() && backUpGridScheduler.isPresent()) {
                    ...
                    Task<Void> monitorTask = Task.action(() -> {
                        ...
                        MonitoringRequest monitoringRequest = new MonitoringRequest(id, req, backUpGridSchedulerId);
    ...
    public synchronized void finish(Node node, WorkRequest finished) throws RemoteException {
        if (!running)
            throw new RemoteException("[RM"+id+"] I am offline");
    ...
    public synchronized void start() throws RemoteException {
        ...
        ExecutorService taskScheduler = Executors.newFixedThreadPool(10);
